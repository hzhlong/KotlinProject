...
class SkuView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : FrameLayout(context, attrs, defStyle) {
    ...
    fun getSkuInfo(): String {
        return mSkuTitleTv.text.toString() + GoodsConstant.SKU_SEPARATOR + // zhy:max_select="1"是单选
                mGoodsSku.skuContent[mSkuContentView.selectedList.first()] // selectedList.first()获取第一个选中的item
    }
}
