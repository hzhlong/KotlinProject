...
class CartFragment : BaseMvpFragment<CartListPresenter>(), CartListView {
    ...
    private var mTotalPrice: Long = 0 // 总价
    ...
    private fun updateTotalPrice() {
        mTotalPrice = mAdapter.dataList
                .filter { it.isSelected } // 过滤出哪些被选中的，然后把数据往下传递【相当于 if(it.isSelected) return it】
                .map { it.goodsCount * it.goodsPrice } // 把过滤的it映射出来，进行操作，然后把数据往下传递
                .sum() // 把每个it叠加
     ...
     *  提交购物车回调
     */
    override fun onSubmitCartListResult(result: Int) { // 返回订单号
